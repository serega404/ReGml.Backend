...
SSH_WAIT_ATTEMPTS="${SSH_WAIT_ATTEMPTS:-72}"
GUEST_INSTALL_DIR="${GUEST_INSTALL_DIR:-/tmp/regml-manager-ci-${VM_DISTRO}-${VM_ARCH}}"
SSH_USER="${SSH_USER:-root}"
GUEST_REPO_DIR="/root/repo"
QEMU_ALLOW_TCG="${QEMU_ALLOW_TCG:-false}"
QEMU_MEMORY_MB="${QEMU_MEMORY_MB:-8192}"
QEMU_SMP="${QEMU_SMP:-4}"
DEBIAN_CLOUD_IMAGE_BUILD="${DEBIAN_CLOUD_IMAGE_BUILD:-latest}"
QEMU_PID=""
...
print_qemu_logs() {
    ...
    for log_file in "${QEMU_STDERR:-}" "${QEMU_STDOUT:-}" "${SERIAL_LOG:-}"; do
...
install_host_dependencies() {
    ...
    case "$VM_ARCH" in
        amd64|x86_64)
            ...
            required_commands="curl ssh tar qemu-img cloud-localds qemu-system-x86_64"
            ;;
        arm64|aarch64)
            packages="$packages qemu-system-arm qemu-efi-aarch64"
            required_commands="curl ssh tar qemu-img cloud-localds qemu-system-aarch64"
            ;;
        *)
    ...
    if [ "$VM_ARCH" = "arm64" ] || [ "$VM_ARCH" = "aarch64" ]; then
    ...
    if [ "$missing" != "true" ]; then
        log "Host QEMU dependencies are already available"
        return 0
    fi
...
resolve_image() {
    case "$VM_DISTRO:$VM_ARCH" in
        debian-13:amd64|debian-13:x86_64)
            ...
            IMAGE_URL="https://cloud.debian.org/images/cloud/trixie/$DEBIAN_CLOUD_IMAGE_BUILD/$IMAGE_FILE"
            CHECKSUM_URL="https://cloud.debian.org/images/cloud/trixie/$DEBIAN_CLOUD_IMAGE_BUILD/SHA512SUMS"
        ...
        alpine-3.23:x86_64|alpine-3.23:amd64)
            IMAGE_FILE="nocloud_alpine-3.23.4-x86_64-bios-cloudinit-r0.qcow2"
        ...
        alpine-3.23:aarch64|alpine-3.23:arm64)
            IMAGE_FILE="nocloud_alpine-3.23.4-aarch64-uefi-cloudinit-r0.qcow2"
            IMAGE_URL="https://dl-cdn.alpinelinux.org/alpine/v3.23/releases/cloud/$IMAGE_FILE"
            CHECKSUM_URL="$IMAGE_URL.sha512"
            CHECKSUM_KIND="alpine"
            ;;
...
download_and_verify_image() {
    ...
    if [ "$actual" != "$expected" ]; then
        log "Cached image checksum mismatch, downloading a fresh copy"
        curl -fL --retry 5 --retry-delay 5 "$IMAGE_URL" -o "$BASE_IMAGE"
        actual="$(sha512sum "$BASE_IMAGE" | awk '{ print $1 }')"
...
users:
  ...
  - name: root
    lock_passwd: false
    plain_text_passwd: regml-ci-root
    ssh_authorized_keys:
      - $public_key
runcmd:
  - printf '\n# ReGml Manager CI\nPermitRootLogin yes\nPubkeyAuthentication yes\n' >> /etc/ssh/sshd_config
  - if command -v rc-service >/dev/null 2>&1; then rc-service sshd restart; else systemctl restart ssh || systemctl restart sshd; fi
ssh_pwauth: false
disable_root: false
EOF
...
start_vm() {
    ...
    if [ ! -e /dev/kvm ] && [ "$QEMU_ALLOW_TCG" != "true" ]; then
    ...
    case "$VM_ARCH" in
        ...
        arm64|aarch64)
            ...
            if [ "$QEMU_ALLOW_TCG" = "true" ]; then
                machine="virt,accel=tcg"
                cpu="${QEMU_CPU:-cortex-a72}"
                log "Using QEMU TCG emulation for ARM guest"
            fi
...
wait_for_ssh() {
    ...
    for attempt in $(seq 1 "$SSH_WAIT_ATTEMPTS"); do
        ...
        if [ "$((attempt % 12))" -eq 0 ]; then
            log "Still waiting for SSH after $((attempt * 5)) seconds"
            print_qemu_logs
        fi
...
run_guest_scenario() {
    ...
    guest_compose_url="file://$GUEST_REPO_DIR/docker-compose-prod.yml"
    remote_env="INSTALL_DIR=$(single_quote "$GUEST_INSTALL_DIR") INSTALL_VERSION=$(single_quote "$INSTALL_VERSION") UPDATE_VERSION=$(single_quote "$UPDATE_VERSION") REPO_DIR=$(single_quote "$GUEST_REPO_DIR") COMPOSE_URL=$(single_quote "$guest_compose_url")"
