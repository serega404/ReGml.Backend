    ...
    as_root sh -c '
        ...
        for attempt in $(seq 1 60); do
            ...
            if [ -n "$container_ids" ]; then
                docker inspect -f "{{ index .Config.Labels \"com.docker.compose.service\" }} {{.State.Status}} {{.State.Restarting}} {{.State.ExitCode}}" $container_ids > "$status_file" 2>/dev/null || true
            fi

            for service in $services; do
                total_services=$((total_services + 1))
                state="$(awk -v service="$service" "\$1 == service { print \$2, \$3, \$4; found = 1; exit } END { if (found != 1) exit 1 }" "$status_file" 2>/dev/null || true)"
